...
// #region Модулі в ES6
/*
Є кілька варіантів ЕКСПОРТ:
  1.
  export let one = 1;

  2.
  let two = 2;
  export {two}; - іменований синтаксис

  Так само 2а способами можемо експортувати функції.
  Важливо, щоб у нас були назви.
  Все буде добре якщо в одному файлі буде кілька експортів

  3. Експорт по замовчуванню
  export default function sayHi() {
    console.log('Hello');
  };


Є кілька варіантів ІМПОРТ
  1.
  import {one, two} from './path';

  При імпорті можна перейменувати те що імпортуємо:
    import {one as first} from './path';

  2. Імпортувати абсолютно все з одного файлу (коли у нас багато 
     експортів в одному модулі):
  import * as data from './path';

  Щоб використовувати зміні чи функції які ми так імпортували потрібно
  викликати їх через крапку:
    console.log(data.one);

  Це працює тому що ми по факту імпортуємо обʼєкт.

  3. Імпорт дефолтної сутностні
    import sayHi from './path';

    Дефолтне значення може бути лише одне, і це дозволяє отримати його
    напряму без фігурних дужок.

Таку модульну структуру все рівно має збирати збірник типу webpack


Додаткова інформація. 
Можна змусити браузер збирати модульний проект, для цього 
правильно підключаємо скрипти:
  Спочатку підключаєтсья файл з якого йде експорт:
    <script type='module' src="./js/main.js"></script>
    А тепер основний:
    <script type='module' src="./js/index.js"></script>

    Важливо прописати: type='module', це змусить браузер
    послідовно підключати файли. Але це не те саме що робить webpack
    чи інший збірник... Ну і якщо файлів буде багато, то буде багато
    підключених скриптів.

    Крім цього при імпорті модулів в основний файл, потрібно повністю
    прописувати шлях із форматом файлу.

    Цим способом дуже рідко користуються.
...
У великих проектах звичайна практика розбивати все не модулі, тобто 
на окремі файли. Щоб функції з одного файли були доступні в інших, 
їх потрібно експортувати. Для цього в кінці скрипта:
  module.exports = назва функції;

А щоб використовувати функції які було експортовано в інших файлах
потрібно імпортувати функцію в потрібний файл:
  const myModule = require('шлях до файлу'); - шлях до файлу прописуємо 
    не вказуючи формат файлу, тому що зборщик сам все зрозуміє

  Уявімо що в імпортованому файлі була функція конструктор, тому, далі
  ми можемо просто з нею працювати:
  ...
  !!! Важливо
  Браузер нічого не знає про експорти, імпорти і він не вміє збирати 
  модулі.

Тому люба модульна система має бути зібрана в один результуючий файл.
Саме тому нам потрібен збрірник, одним із дуже популярних є Webpack.
Вона вміє не тільки збирати скрипти до купи, але і конфігурувати
проект: обробляти стилі, картинки, формувати папки і ще кучу всього.

Не путати з Gulp, який є планувальником задач.

Щоб працювати з Webpack потрібно його встановити:
  - mkdir webpack-demo // створюємо папку
  - cd webpack-demo // переходимо в цю папку
  - npm init -y // ініціалізуємо npm проект
  - npm install webpack webpack-cli --save-dev // встановлюємо webpack

Є два режими роботи
1. умовно авто, те як його налаштували розробники
2. ручний режим, за допомогою конфіг файлу

1. Щоб працював авто режим:
  Потрібно, щоб основний файл знаходився в папці src і називався
  index.js
  
Більше деталей: https://webpack.js.org/configuration/mode/
...
// #region 101 Модуль
/*
Якщо провести аналогію з книгою, то модуль це глава.

Переваги використання:
1. Всі модулі по своїй структурі самодостатні і незалежні
2. Забезпечує чистоту глобальної області.
   Коли просто створюємо змінні, вони заносяться в глобальну область. І робити це
   не рекомендується, змінних в глобальній області повинно бути як можна менше.
   Відповідно модулі в цьому допомагають і більше того, допомагають уникнути
   плутанини з однаковими назвами, тому що всі дані модуля існують лише в його області
   видимості.
3. Допомагає легше переносити модуль з одного проекту в інший.

На практиці є 2 способи:
  1. Використати анонімну само-викликаючу функцію:
    Синтаксис виглядає так: (function(){}());
      - Зовніжні дужк потрібні щоб не було function declaration (яка не може бути 
        анонімною). А завдяки цим дужкам у нас function expression
      - Далі звичайний синтаксис функції
      - І після закриття фігурної дужки, використовуємо () - щоб відразу визвати функцію
    ...
    В коді це може виглядати так:
      ...
      (function(){
        let number = 2;
        console.log(number); // отримаємо 2
      }())

      console.log(number); // отримаємо 1

  2. Використання обʼєктного інтерфейсу
    const user = (function(){
    ...
    user.sayHello(); // отримаємо I am privat

*/
// #endregion

// #region 100 Інкапсуляція
/*
Інкапсуляція - це один з принципів ООП, ціль якого відділити і 
  приховати код від зовнішнього світу. В ООП це означає що обʼєкт 
  зберігає свій стан в приватному порядку і тільки методи обʼєкта
  мають доступ до його змін. В JS це можна лише імітувати.
...
Приклад на функції:
  ...
  function User(name, age) {
    ...
    this.say = function() {
      console.log(`Імʼя користувача: ${this.name}, вік ${this.age}`);
    };
  };
  ...
  // Але так само можна і змінити дані:
  ivan.name = 'Alex';
  ivan.age = 30;

  // Тепер викликаємо метод .say:
  ivan.say(); 
  відповідно любий зможе таким чином втрутитись і змінити обʼєкт
  саме тому потрібна інкапсуляція.


  // А якщо вік задати через let
  function User(name, age) {
    this.name = name;
    let userAge = age;

    this.say = function() {
      console.log(`Імʼя користувача: ${this.name}, вік ${userAge}`);
    };
  };

  const ivan = new User('Ivan', 27);
  console.log(ivan.name);
  console.log(ivan.userAge); // результат undefined

  // Але так само можна і змінити дані:
  ivan.name = 'Alex';
  ivan.userAge = 30; // Не спрацює

  // Тепер викликаємо метод .say:
  ivan.say(); // відповідно видасть: Імʼя користувача: Іван, вік 27

  Все тому що змінна userAge створенна всередині обʼєкта івідповідно 
  вона не доступна ззовні. Щоб працювати з такими змінними потрібні
  геттери і сеттери.


  // Отже щоб додаємо геттер і сеттер
  function User(name, age) {
    this.name = name;
    let userAge = age;
  ...
  // Тепер у нас є 2 методи які дозволяють отримати і записати значення
  const ivan = new User('Ivan', 27);
...
Все те саме в класах:
  class User {
    ...
    say() {
      console.log(`Імʼя користувача: ${this.name}, вік ${this.userAge}`);
    }
  ...
  // Тепер вік можна змінити 2-а способами
  ivan.userAge = 99;
  ...
  Якщо потрібно скрити від зовнішніх факторів, то існує домовленість
  перед назвю писати нижній прочерк, коли ми оголошуємо властивість:
  this._userAge = age; 

  !!!Важливо це не синтаксис JS, а просто домовленість

  А щоб правильно керувати цими властивостями потрібно використовувати
  getter & setter:
    class User {
      constructo(name, age) {
        this.name = name;
      ...
      say() {
        console.log(`Імʼя користувача: ${this.name}, вік ${this._age}`);
      }
    ...
    Так як ми використовуємо геттер і сеттер:
    ivan.age = 99;
    console.log(ivan.age);

    !Важливо. Так як _age всього домовленість, то ми все ще можемо змінвати значення
    звичайним способом:
     ivan._age = 20;
     console.log(ivan._age);

     Але таке використання не правильне...
    

  Поля класів, дозволяються заборонити зміну ivan._age = 20 в класах. Але на момент
  запису уроки це був експерементальний функціонал. ПОТРІБНО ПЕРЕВІРИТИ
  ...
  ---- Поля класів -------------------------------
    class User {
      ...
      surname = 'Seleznov'; // Можна записувати властивість за межами конструктора

      // 2-e методи в класах можна задавати у вигляді стрілочної функції
      say = () => {
        console.log(`Імʼя користувача: ${this.name} ${this.surname}, вік ${this._age}`);
      }
  ...
  !!! Про варіант "Поля класів" більш детально в частині про реакт. І в JS це ще не 
  прийнятий стандарт, але можливість ізоляції є, потрібно просто додати # перед
  властивістю:
    class User {
      constructo(name, age) {
        this.name = name;
      ...
      #surname = 'Seleznov'; // додаємо # і властивість не можна змінити напряму іззовні
      // 2-e методи в класах можна задавати у вигляді стрілочної функції
      say = () => {
        console.log(`Імʼя користувача: ${this.name} ${this.#surname}, вік ${this._age}`);
      }
    ...
    Відповідно тепер якщо спробуємо викликати властивість то:
    console.log(user.surname); - отримаємо undefined
    console.log(user.#surname); - отримаємо помилку

========================================================================

Домашня робота: написати геттер і сеттер для surname:
  class User {
    constructo(name, age) {
      this.name = name;
      this._age = age;
...
// #region 99 Геттери і Сетери
/*
Обʼєкт слкдається з властивостей і методів.

А властивості діляться ще на 2 категорії:
- Властивості "Дані" - все що до цього моменту використовував
- Властивості "Аксесори" - це цілий комплекс який дозволяє як 
  присвоювати так і отримувати значення. Але при цьому в коді
  виглядають, як звичайні властивості

Аксесори поділяються на 2 великі групи:
  - Гетери
  - Сетери
...
Приклад Getter:
  ...
  Опис:
    - Синтаксис починаєтсья з ключового слова get
    - Далі вказуємо назву
    - Коли ззовні визиваємо, не ставимо (), тому що гетер і так означає
      що всередині якийсь функціонал і він просто дозволяє нам працювати
      з цим методом як зі звичайною властивістю;
...
Приклад Setter:
  ...
  Опис:
    - Починаємо із ключового слова set
    - Вказуємо назву, як може бути такою самою як у get, тому що 
      вони взаємо виключаючі і можуть працювати в парі.
    - Відповідно сетер дозволяє легко перезаписати дані в обʼєкті
    - Результут обох console.log 30, в першому ми змінили занчення, 
      а в другому вивели вже оновлене значення.

*/
// #endregion 

// #region 96 Ругулярні вирази
/*
Ругулярні вирази - технологія яка дозволяє зручніше працювати із рядками:
  видаляти, замінювати частини слів, шукати якісь частинки рядка, обмужувати
  ввод певних знаків і тд.

Ця технологія використовується не лише в JS.

Всі регулярні вирази складаються з 2-х частин: патерн і флаги
  патерн - це шаблон, наприклад того що ми шукаємо, або що ми намагаємось видалити
  флаги - 

Синтаксис створення:
  new RegExp('patern', 'flags'); - класичний метод створення, яким ніхто 
                                   не користується.

  /patern/flags; - більш зручний запис

Флаги:
  i - незалежний від регістру пошук
  g - шукаємо кілька значень
  m - багаторядковий режим

Практика: 

Метод .search():
  const ans = prompt('Вкажіть ваше імʼя');

  const reg = /n/;

  console.log(ans.search(reg));
    В результаті ми отримаємо перший знайдений індекс по якому знаходиться 
    символ n у відповіді користувача. Якщо не знайдемо, то результат буде -1.

  Для більш детального пошуку використовуються флаги.
  Наприклад нам е важливий індекс:
    const reg = /n/i;
...
Метод .match():
  ...
  console.log(ans.match(reg));
    В результаті отримаємо масив, в якому є: кусок рядка, який шукали;
    індекс; сам рядок в якому відбувався пошук; властивість groups

  Але якщо нам потрібно знайти всі букви n у рядку:
    const reg = /n/ig;
    console.log(ans.match(reg)); 
      Отримаємо масив де будуть всі букви n які увів користувач.
      При цьому в масив будуть додавати і великі N, і малі n, бо 
      стоїть флаг і.

Метод .replace():
Приймає 2 аргументи: що ми змінюємо і на що змінюємо:

  const pass = promt('Password');

  console.log(pass.replace(/./g, '*'));
    Крапка означає що ми беремо всі елементи які попадуть в рядок.
    g - знак глобальності, щоб всі символи були взяті
    * - другий аргумент, відповідно всі символи будуть замінені на *

  Якщо потрібно замінити крапку на *, то виразу буде такий:
    console.log(pass.replace(/\./g, '*')); - \ - означає що далі йде не спецсимвол. (екранирование)

  Ругулярний вираз не обовʼязково записувати у змінну, як це було вище.
  Деталі про спецсимволи і що вони означають: https://developer.mozilla.org/ru/docs/Web/JavaScript/Reference/Global_Objects/RegExp#character-classes

  Ще приклад з .replace:
    console.log('12-34-56'.replace(/-/g, ':')); 
      відповідно в консоль виведе: 12:34:56


В регулярних виразів є свої методи:
  const ans = prompt('Вкажіть ваше імʼя');
  const reg = /n/i;

  console.log(reg.test(ans)); - поверне або true або false

В регулярних виразах можна шукати цілий клас символів:
  \d - шукає числа 
  \w - всі букви 
  \s - всі пробіли
...
Комбінації класів:
  ...
  console.log(str.match(/\w\d\w\d/i));
    таким чином ми отримаємо масив де 0й індекс, це R2D2 бо ми 
    шукали комбінація із класів: буква, цифра, буква, цифра

*/
// #endregion

// #region 95 localStorage
/*
localStorage - дозволяє зберігати інформація навіть без роботи з сервером 
і БД. І вся інформація залишиться навіть якщо перезавантажити сторінку
чи закрити браузер.

Може помістити в себе до 5Мб інформації.

По суті це об‘єкт, який вбудований в браузер і він може зберіга різні дані.
Якщо копати глибше, то це властивість глобального обʼєкта window.

Вважливо! Цей обʼєкт унікальний для кожного домену.

В консолі розробника, вкладка Application.

Основні команди:
  localStorage.setItem('key', 'value'); // створить властивість і значення, або перезапише якщо вже є.

  localStorage.getItem('key') // отримуємо значення властивості
  
  localStorage.removeItem('key') // видаляє властивість

  localStorage.clear() // очищаємо сховище повністю.


Практика:
  Завдання, у нас є проста форма з полем, чекбоксом і кнопками. Наше
  завдання зробити так, що якщо користувач натиснув на чекбос, то після
  оновлення сторінки чекбокс має залишитись. Друге, потрібно зробити
  так, щоб користувач міг перекрасити форму і при оновлені це не зникало

  Спочатку отримуємо уявні елементи:
...
Ще приклад:
  ...
  console.log(JSON.parse(localStorage.getItem('alex'))); - отримаємо звичайний обʼєкт

  Без serializedPersone, у нас в localStorage запишеться [object, Object], все
  тому що при записі обʼєкт перетворився в рядок... Динамічна типізація

*/
// #endregion

// #region 91 Бібліотеки. Бібліотека axios
/*
Бібліотека - це якась готове рішення, яке можна перевикористати. Це може 
бути навіть 1 простий метод і щось дуже велике.

Підключення на прикладі axios:
  - При максимально правильних обставинах, встановлюється як npn пакет
    npm install axios

    але цей спосіб буде розглянутий пізніше

  - Другий спосіб, просто беремо скріпт і вставляємо його в наш html перед 
    перед нашим основним скриптом. При цьому і тут є два варіанти:
      1. Скачати файл скрипта і додати в наш проект
      2. Посилання на CDN. Можна взяти на ресурсі: https://cdnjs.com/

Сам axios - це ресурс який дозволяє зручно працювати з серевером
  в неї відразу додано багато додаткових можливостей, які без
  бібліотеки приходилось би прописувати самому. Як приклад в приктичному
  робили перевірку і викидували помилку в консоль вручну. З підключиеною
  бібліотекою цього можна не робити, бо вона сама це зробить


*/
// #endregion

// #region 90 async & await
/*
Завжди є сенс виносити функціонал по спілкуванню з серевером
у окремі функції.

async & await на практиці:

  В проекті є функція спілкування з серевером
  ...
  Важливо розуміти, що все це асинхронний код:
    Коли запускається функція postData, то в першому рядку ми починаємо 
    робити запит і fetch це завжди асинхронний код.

    Отже після цього, ми хочемо повернути наш res... 
    
    Але виходить що ми відправили запит через fetch, створили пусту
    змінну res... важливо що, ще не отримали відповідь... але 
    намагаємось повернути res, який по факту ще пустий...

  Тому потрібно перетворити асинхронний код в синхронний (по факту в симуляцію синхронного коду).
  Саме для цього потрібен async & await, які завжди використовуються 
  в парі:
  ...
  Отже за допомогою async ми кажемо, що всередині функції буде 
  якийсь асинхронний код.
  А await ставиться перед тими операціями які нам потрібно дочекатись,
  щоб продовжити виконання коду.
    

Додаткова інформація. Перетворюємо дані в JSON:

  Для початку основа:
  Є метод Object.entries(obj); Який перетворює обʼєкт в масив масивів

  Отже тепер ми можемо в нашому проекті:
    const json = JSON.stringify(formData.entries())...

  Але щоб правильно трансормувати, нам потрібно також перетоврити 
  цей масив масивів знову в обʼєкт:
    const json = JSON.stringify(Object.fromEntries(formData.entries()));
  
    Тепер виходить що ми беремо formData яка зібрала всі дані з форми,
    перетворюємо в масив масивів і назад в обʼєкт, який ми вже перетворюємо
    в JSON/

*/
// #endregion

// #region 89 npm + json-server
/*
npm-пакети - це окремі кусочки коду які лежать на серверах і які
ми можемо встановлювати собі в проект.

Інструкція:
  1. Спочатку потрібно "сказати" нашому проекту, що він міститиме
  npm пакети - відповідно він стане npm проектом:
    npm init

  2. Проходимо опитувальник
    - Даємо назву проекту
    - Версія
    - Опис
    - Основний файл (entry point)
    - Команда для тестів
    - git repository
    - Ключові слова
    - Автор
  ...
  Після застосування змін, у нас з‘являється файл: packege.json,
  який збереігає всю інформацію про проект і найголовніше він буде 
  зберігати інформацію про встановлені npm пакети

  3. Встановлюємо npm пакети на прикладі json-сервер:
    npm install json-server
    
    або ще можна install замінити просто на і:
      npm i json-server

    // пакети можна встановлювати глобально і локально, рекомендується
       встановлювати локально, бо інформація про локальні пакети
       записується в наш packege.json, а глобальні ні.
       Щоб встановити пакет глобально додаємо -g (і для mac sudo, 
       тому що потрібні права андміністратора):
        sudo npm i json-server -g

    Крім того, нам потрібно вказати флаг який означає, як цей пакет
    впливає на проект (приклади: використовується лише при розробці
    чи використовується при роботі проекту). Це робиться 2-а різними 
    командами:
      1. npm i json-server --save-dev // це залежність для розробки
      2. npm i json-server --save // відповідно показує що залежність
            потрібна для роботи всередині, як приклад бібліотека
            react чи jquery

    У випадку з json-server потрібно --save-dev

    Як результат буде створено папку node_modules.

    !!! Ніколи не чіпати руками папку node_modules

    Папка node_modules може займати сотні Мб і її не потрібно пушити
    на гіт. Для цього потрібно використовувати файл .gitignore

    Ще особливість папка node_modules - видаляється лише за допомогою 
    спеціальної команди.

    Реальний кейс:
      Ми завантажуємо з гіта новий проект, наприклад бачимо що є
      файл package.json але немає папки node_modules... щоб встановити
      всі необхідні пакети, є команда:
        npm i 

      Яка встановить відразу всі пакети які зафіксовані в файлі 
      package.json

      
    Також разом з package.json зʼявляється файл package-lock.json
    Його можна не чіпати і там записані всі залежності.

----------------------------------------------------------------------

JSON-СЕРВЕР
Це простий сервер по роботі з json файлами, які можна використовувати
як не великі бази даних. Головне що він працює з багатьма методами в 
тому числі з POST

Щоб запустити json-сервер, в терміналі:
  npx json-server db.json // db.json наш файл з базой
...
Приклад
  ...
  Примітка:
    Формує новий масив з відфльтрованими даними
    Обовʼязково має бути return, без нього не сформується масив

    Так як функція дуже проста все можна записати в 1 рядок:
        const filteredNames = names.filter(name => name.length < 5)


-----------------------------------------------------------------------
.map
Дозволяє взять кожен окремий елемент масив і змінити його, при цьому
як результат отримаємо новий масив із зміненим елементами.

Синтаксис такий самий:
  const answers = ['IvAn', 'AnnA', Hello'];

  const result = answers.map(item => item.toLowerCase());

  Примітка:
    Як результат отримає новий масив з усіма елементами але в нижньому 
    регістрі.

    В цілому не обовʼязково створювати новий масив, можна перезаписати
    старий, якщо в цьому є необхідність. Але з точки зору програмування
    і такого поняття як іммутабельність (пояснення буде далі по курсу),
    краще створювати новий масив


-----------------------------------------------------------------------
.every / .some
Обидва перевіряють масив на наявність чогось, що нам потрібне і 
повертають бульове значення. 

every - віддає true лише якщо всі елементи масиву відповідають 
        критеріям

some - віддає true лише якщо хоч один елементи масиву відповідають 
       критеріям
...
-----------------------------------------------------------------------
.reduce
Потрібен для того щоб обʼєднувати дані в середині масиву

Приклад, нам потрібна сума всіх данних в масиві:
const numbs =  [4, 5, 1, 3, 2, 6];

const res = numbs.reduce((sum, current) => sum + current);

  Цикл по якому проходиться reduce буде виглядати так:
    0    4
    4    5
    9    1
    10   3
  ...
  Відповідно результат буде 21. Фактично перше коло ігнорується, тому
  приклад з текстом. Нам потрібно отримати список через кому:
    const fruits = ['apple', 'orange', 'pear', 'plum'];

    const list = fruits.reduce((sum, current) => sum + ', ' + current);
    або
    const list = fruits.reduce((sum, current) => `$(sum), ${current}`);
...
Крім цього reduce може приймати ще один аргумент, приклад:
  const numbs =  [4, 5, 1, 3, 2, 6];

  const res = numbs.reduce((sum, current) => sum + current, 3);

  Додавши 3, ми вказуємо початкове значення, отже на першому колі у нас
  буде:
    3    4
    4    5
    9    1
    ...
  Як результат отримаємо не 21, а вже 24.

  Те саме можна зробити і з рядками.


-----------------------------------------------------------------------
Практика:
Уявимо що у нам прийшов обʼєкт з сервера в дивному форматі:
  const obj = {
    ivan: 'person',
    ann: 'person',
    dog: 'animal',
...
і нам потрібно отримати імена людей (при цьому ми не знаємо де вони
можуть знаходитись). 

Послідовність:
- Перетворюємо наш обʼєкт на масив масивів, за допомогою .entries;
- Створюємо масив в якому буть лише ті масиви в яких за індексом 1
  'person' за допомогою filter;
- Створюємо масив із 0 елементу в кожному нашому масиві за допомогою
  .map

Всі операції можна записати цепочкою (chain), не рекомендується часто
робити такий запис, але в даному випадку так зручніше:
...
Відповідно тепер можна навіть застосувати reduce для вивода всіх імен
наприклад через кому, як робили в прикладі з reduce вище.
