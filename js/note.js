...
Якщо нам потрібно отримати "скільки користувач вже проскроли сторінку":
  document.documentElement.scrollTop; // або так само все інше, як описано вище для box

  Відповідно якщо ми хочемо проскролити сторінку:
    document.documentElement.scrollTop = 0; // сторінка проскролиться на початок.

  Також для скрола сторінки у window є scrollBy & scrollTo:
    window.scrollBy(x, y) - це працює від поточної позиції, тобто якщо задачати 'y' як 100, то при кожному спрацюванню буде скролитись на 100 px вниз
    window.scrollTo(x, y) - в цьому випадку скролиться до точних координат, і при кожному спрацюванню скрол буде в одне місце.
...
Можна додавати скільки завгодно класив:
.classList.add('red', 'black');

Класи можна використовувати в умовах:
.classList.contains('red'); - повертає ульове значення: true - такий клас є
